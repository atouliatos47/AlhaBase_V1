...
@app.post("/data/set")
async def set_data(
    item: DataItem, username: str = Depends(verify_token), db: Session = Depends(get_db)
):
    if not security_rules.validate_write(item.collection, username):
        raise HTTPException(
            status_code=403,
            detail=f"Write access denied to collection: {item.collection}",
        )
    ...
    if existing_data:
        ...
        if not security_rules.validate_write(item.collection, username, resource_data):
            raise HTTPException(
                status_code=403, detail="Not authorized to update this data"
            )
    ...
    await manager.broadcast(
        json.dumps({"action": "update", "collection": item.collection, "key": item.key})
    )
    return {
        "success": True,
        "collection": item.collection,
...
async def get_data(
    ...
    if not security_rules.validate_read(collection, username):
        raise HTTPException(
            status_code=403, detail=f"Read access denied to collection: {collection}"
        )
...
async def query_data(
    ...
    if startAfter:
        query_params["startAfter"] = startAfter
    ...
    # Press Events Endpoints


@app.post("/api/collections/press_events/records")
async def create_press_event(
    request: dict, username: str = Depends(verify_token), db: Session = Depends(get_db)
):
    """
    Create a new press event record
    """
    try:
        # Generate a unique key for this event
        event_key = f"event_{datetime.utcnow().strftime('%Y%m%d_%H%M%S_%f')}"

        # Create the data item
        data_item = DataItem(collection="press_events", key=event_key, value=request)

        # Use existing data/set endpoint logic
        if not security_rules.validate_write(data_item.collection, username):
            raise HTTPException(
                status_code=403,
                detail=f"Write access denied to collection: {data_item.collection}",
            )

        data_id = f"{data_item.collection}:{data_item.key}"
        existing_data = db.query(DataDB).filter(DataDB.id == data_id).first()

        if existing_data:
            resource_data = {"owner": existing_data.owner, "id": existing_data.id}
            if not security_rules.validate_write(
                data_item.collection, username, resource_data
            ):
                raise HTTPException(
                    status_code=403, detail="Not authorized to update this data"
                )
            existing_data.value = json.dumps(data_item.value)
            existing_data.owner = username
        else:
            new_data = DataDB(
                id=data_id,
                collection=data_item.collection,
                key=data_item.key,
        ...
        return {
            ...
            "key": data_item.key,
            "id": event_key,
            "message": "Press event recorded successfully",
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to create press event: {str(e)}"
        )
...
async def list_collections(
    ...
    try:
        ...
        return {"success": True, "collections": sorted(collections)}
    except Exception as e:
...
async def delete_data(
    ...
    if not security_rules.validate_write(collection, username):
        raise HTTPException(
            status_code=403, detail=f"Write access denied to collection: {collection}"
        )
    ...
    db.commit()
    await manager.broadcast(
        json.dumps({"action": "delete", "collection": collection, "key": key})
    )
...
async def upload_file(
    ...
    file_info = await file_storage.save_upload_file(
        file, username, is_public.lower() == "true"
    )
    file_record = FileDB(
...
async def download_file(
    file_id: str, username: str = Depends(verify_token), db: Session = Depends(get_db)
):
...
async def update_security_rule(
    collection: str, rules: dict, username: str = Depends(verify_token)
):
...
async def send_email_notification(request: dict, username: str = Depends(verify_token)):
...
async def send_alert_notification(request: dict, username: str = Depends(verify_token)):
...
async def update_email_settings(request: dict, username: str = Depends(verify_token)):
    ...
    try:
        ...
        current_config["email"]["smtp_server"] = request.get(
...
async def get_telegram_settings(username: str = Depends(verify_token)):
    ...
    safe_config = {
        ...
        "bot_token": (
            telegram_config.get("bot_token", "")[:20] + "..."
...
async def update_telegram_settings(
    request: dict, username: str = Depends(verify_token)
):
...
async def send_telegram_notification(
    ...
    return {
        "success": success,
        "message": (
            "Telegram sent successfully" if success else "Failed to send Telegram"
        ),
    }
